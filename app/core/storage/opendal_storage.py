...
def _get_opendal_kwargs(
    *, scheme: str, env_file_path: str = ".env", prefix: str = "OPENDAL_"
):
    kwargs = {}
...
class OpenDALStorage(BaseStorage):
    def __init__(self, scheme: str = "fs", **kwargs):
        kwargs = kwargs or _get_opendal_kwargs(scheme=scheme)

        if scheme == "fs":
            root = kwargs.get("root", settings.OPENDAL_ROOT)
